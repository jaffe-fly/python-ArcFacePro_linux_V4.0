...
class Image_dect():
    ...
    # 人脸属性检测
    def Process(self,mask=Face_read_so.PROCESS_MASK):
...
if __name__ == '__main__':
    ...
    res = imde.Process()
    ageinfo = imde.getAge()
    print("检测到的人脸年龄：",ageinfo[1][0])
    genderinfo = imde.getGender() #0:男性; 1:女性; -1:未知
    print("检测到的人脸性别：",genderinfo[1][0])
    get3Dangle = imde.get3Dangle()
    # 不支持f-string格式化
    # TODO status数值显示不正确，待调试
    print("3DAngle roll: {:.2f}\n".format(get3Dangle[1].roll[0]),
          "3DAngle yaw: {:.2f}\n".format(get3Dangle[1].yaw[0]),
          "3DAngle pitch: {:.2f}\n".format(get3Dangle[1].pitch[0]),
          "3DAngle status: {}\n".format(get3Dangle[1].status),
          "3DAngle num: {}".format(get3Dangle[1].num))

    quality = imde.ImageQualityDetect()
    print("检测到的人脸质量：",quality[1])
